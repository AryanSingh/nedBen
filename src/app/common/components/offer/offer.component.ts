...
module nedben.directives.offerDirective {
	'use strict';

	class OfferDirectiveController {
		static $inject = ['$log', '$http', 'ENV', 'OFFER_MANAGEMENT_URLS','IMAGES_URLS', '$rootScope','Socialshare','$location','$anchorScroll','session'];
		...
		constructor($log:ng.ILogService, $http:ng.IHttpService, ENV:any, OFFER_MANAGEMENT_URLS:any,IMAGES_URLS:any,
			$rootScope:ng.IRootScopeService, Socialshare:any,$location:ng.ILocationService,$anchorScroll:ng.IAnchorScrollService,session:any) {
			this.$log = $log;
			this.$http = $http;
			this.ENV = ENV;
			this.IMAGES_URLS=IMAGES_URLS;
	...
	angular.module('nedben.directives').directive('offer', OfferDirective.factory());
}
