...
export class ProfileAvatarComponent implements OnInit {
		...
		if(this.session.getUserData()){ 
			if(this.imageSource==='WEB'){
        		...
        		headers.append("Content-Type": undefined);
					this.http.post(this.ENV.apiBasePath+this.USER_MANAGEMENT.UPLOAD_AVATAR,
	...
	removeAvatar(avatarIndex){
		if(this.session.getUserData()){ 
			let data:any = { idAvatar: this.savedAvatar[avatarIndex]['idAvatar'] };
			let headers = new Headers();
			let url = this.ENV.apiBasePath+this.USER_MANAGEMENT.REMOVE_AVATAR;
        	headers.append('Content-Type': 'application/json');
			// this.http.delete(url, data, headers)
			// problem here delete does not take body
